...
class NotesPage(Region):
    ...
    NOTE_LIST_REGISTRY = CustomList(By.CSS_SELECTOR, '[sbisname="notes"]', 'Заметка в реестре')
    NOTE_HOVER_DELETE_BUTTON = Element(By.CSS_SELECTOR, '[sbisname="RemoveNote"] i', 'Кнопка удаления заметки по ховеру')
    NOTE_DELETE_SUBMINT_BUTTON = Element(By.CSS_SELECTOR, '[sbisname="positiveButton"] .controls-Button__content', 'Кнопка подтверждения удаления')
    NOTE_SAVE_BUTTON = Element(By.CSS_SELECTOR, '[sbisname="okButton"] .controls-Button__content', 'Кнопка сохранения заметки')
    ...
    def delete_note(self, texts_in_note: str):
        ...
        self.NOTE_LIST_REGISTRY.item(with_text=texts_in_note).mouse_over()
